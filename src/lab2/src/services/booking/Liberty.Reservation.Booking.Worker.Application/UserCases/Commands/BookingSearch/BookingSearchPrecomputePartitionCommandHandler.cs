...
public class BookingSearchPrecomputePartitionCommandHandler(
    ILogger<BookingSearchPrecomputeCommandHandler> logger,
    IUnitOfWork unitOfWork,
    IMapper mapper,
    IHttpClientFactory httpClientFactory,
    IOptions<ServiceSetting> serviceSettingOptions
) : ActionCommandHandlerBase<BookingSearchPrecomputePartitionCommand, (string PartitionKey, bool IsSuccess)>(unitOfWork, mapper)
{
    ...
    protected override async Task<(string PartitionKey, bool IsSuccess)> HandleAsync(
        BookingSearchPrecomputePartitionCommand request,
        CancellationToken cancellationToken
    )
    {
        var payload = request.Payload;
        var personAgeTypes = payload.PersonAgeTypes;
        ...
        var facilityCode = payload.FacilityCode?.Trim();
        ...
        return ($"Facility{payload.FacilityCode}-Site{payload.SiteCode}", true);
    }
