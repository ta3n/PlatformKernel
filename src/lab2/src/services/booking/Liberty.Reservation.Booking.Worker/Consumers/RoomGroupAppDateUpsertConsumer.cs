...
{
    private readonly IFacilityRepository facilityRepository = serviceProvider.GetRequiredService<IFacilityRepository>();
        ...
        try
        {
            ...
            await retryPolicy.ExecuteAsync(
                async () =>
                {
                    ...
                    await RemoveCacheWhenFinishAsync(userCode, hotelModels);
                }
            );
        }
        catch (Exception ex)
    ...
    {
        var facilityCodeCacheKeys = hotelModels
            .Select(x => x.HotelId)
            .Distinct()
            .ToList();
