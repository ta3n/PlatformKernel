...
public class BookingRoomChangePersonsBookingCommandHandlerTest : BaseUnitTest
{
    private List<AppDatePriceOfBookingResponse> CreateMockAppDatePrices()
    {
        return
        [
            new()
    ...
    private BookingPriceRequest CreateBookingPriceRequest(
    ...
    private void SetupCommonMocks(
        ...
        Mock<IBookingPriceService> bookingPriceOfPlanServiceMock,
        Mock<IBookingPriceService> bookingPriceOfRoomServiceMock,
        Mock<ISecurityContextAccessor> securityContextMock,
        bool isInvalidChangePersons = false,
        bool useDailyPerson = true,
        BookingPriceResponse? bookingPriceResponse = null
    )
    {
        ...
        bookingCheckAvailableServiceMock.Setup(
            ...
            .ReturnsAsync(isInvalidChangePersons);

        bookingPriceResponse ??= new BookingPriceResponse { AppDatePrices = CreateMockAppDatePrices() };
        ...
        securityContextMock.Setup(x => x.GetFacilityIdSelected()).Returns(1); // Provide a default
    }
    ...
    public async Task HandleAsync_ShouldReturnBookingPriceResponse_WhenAllValid()
    {
        ...
        var expectedBookingPriceResponse = new BookingPriceResponse { AppDatePrices = CreateMockAppDatePrices() };
        var payload = CreateBookingPriceRequest(checkInDate);
    ...
    public async Task HandleAsync_ShouldReturnFacilityNotfoundException()
    {
        ...
        securityContextMock.Setup(x => x.GetFacilityIdSelected()).Throws(new FacilityNotfoundException());
    ...
    public async Task HandleAsync_ShouldReturnPlanNotfoundException()
    {
        ...
        // Act & Assert
        await Assert.ThrowsAsync<PlanNotfoundException>(async () => await handler.Handle(command, cancellationToken));
    }

    [Fact]
    public async Task HandleAsync_ShouldReturnReservationPriceExceptionWhenInvalidChangePerson()
    {
        // Arrange
        var checkInDate = DateTime.Now;
        var securityContextMock = new Mock<ISecurityContextAccessor>();
        ...
        // Act & Assert
        await Assert.ThrowsAsync<ReservationPersonAgeTypeException>(async () => await handler.Handle(command, cancellationToken));
    }
}
