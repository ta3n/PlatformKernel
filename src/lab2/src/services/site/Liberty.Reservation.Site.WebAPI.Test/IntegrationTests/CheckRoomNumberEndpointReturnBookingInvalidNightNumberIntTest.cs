...
public class CheckRoomNumberEndpointReturnBookingInvalidNightNumberIntTest : BaseIntegrationTest
{
    private GetBookingData GetBookingData { get; }
    private const string BaseUrl = "api/booking";

    public CheckRoomNumberEndpointReturnBookingInvalidNightNumberIntTest()
    {
        GetBookingData = new GetBookingData(Factory, Client);
    }

    [Fact]
    public async Task CheckRoomNumber_BookingInvalidNightNumberException()
    {
        ...
        var response = await Client.PostAsync(
            $"{BaseUrl}/plans/1000/rooms/10000/check-room-number",
            request
        );
