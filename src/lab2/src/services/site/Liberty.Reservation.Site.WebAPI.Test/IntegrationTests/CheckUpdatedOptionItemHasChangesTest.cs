...
public class CheckUpdatedEndpointReturnOptionItemHasChangesIntTest : BaseIntegrationTest
{
    private GetBookingData GetBookingData { get; }
    private const string BaseUrl = "api/booking";

    public CheckUpdatedEndpointReturnOptionItemHasChangesIntTest()
    {
        GetBookingData = new GetBookingData(Factory, Client);
    }

    [Fact]
    public async Task CheckUpdated_OptionItemHasChangesException()
    {
        ...
        try
        {
            var exception = JsonConvert.DeserializeObject<OptionItemHasChangesException>(json);
