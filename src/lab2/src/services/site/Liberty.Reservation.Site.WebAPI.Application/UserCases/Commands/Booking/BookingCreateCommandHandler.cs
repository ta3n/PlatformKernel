...
using Liberty.Reservation.Site.WebAPI.Application.UserCases.Events.Booking;
...
public class BookingCreateCommandHandler(
    ...
    IMediator mediator
) : CreateCommandWithAuditEventHandlerBase<BookingCreateCommand, string>(null!, mapper, mediator)
{
    ...
    )
    {
        ...
        BindIsRequiredToQuestions(planQuestions, optionQuestions, existingQuestions);
        ...
        try
        {
            ...
            AuditEventData = new BookingCreatedEvent
            {
                Id = newReservation.Id,
                AggregateCode = newReservation.Code ?? string.Empty,
                Request = request,
                UserCode = newReservation.UserCode,
    ...
    {
        var requiredMap = existingQuestions.ToDictionary(q => q.Id, q => q.IsRequired);
