...
public class SaleExportCsvQueryHandler(
    IMapper mapper,
    IMediator mediator
) : QueryBaseHandler<SaleExportCsvQuery, BookingReservationExportCsvStreamResponse>(mapper)
{
    protected override Task<(IHeaderDictionary, BookingReservationExportCsvStreamResponse)> HandleAsync(
        SaleExportCsvQuery request,
        CancellationToken cancellationToken
    )
    {
        var objectName = "sale_list";
            ...
            (
                ...
                new BookingReservationExportCsvStreamResponse
                {
                    ...
                    ) =>
                    {
                        ...
                        csv.Context.RegisterClassMap<SaleDetailResponseCsvMap>();

                        csv.WriteHeader<SaleDetailResponseCsv>();
                        ...
                        do
                        {
                            ...
                            foreach (var itemCsv in items.Select(item => Mapper.Map<SaleDetailResponseCsv>(item)))
                            {
