...
) : CreateCommandHandlerBase<DestinationCreateCommand, DestinationResponse>(unitOfWork, mapper)
{
    ...
    )
    {
        ...
        var prefixName = payload.PrefixName ?? string.Empty;

        var checkExistingPrefixName = await siteService.CheckExistingPrefixName(
            prefixName,
            [],
            cancellationToken
        );
        if (checkExistingPrefixName)
        {
            throw new SiteDuplicatedPrefixNameException(prefixName);
        }
