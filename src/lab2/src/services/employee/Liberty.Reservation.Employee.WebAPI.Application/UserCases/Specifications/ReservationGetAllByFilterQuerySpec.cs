...
internal class ReservationGetAllByFilterQuerySpec : SpecificationBase<ReservationEntity>
{
    ...
    )
    {
        ...
        {
            AddCondition(
                x => x.CheckInDate.GetDateTime(x.CheckInTime)
                    .AddHourOffset(x.Facility!.TimeZone) >= request.StartCheckInDate.Value.GetDateTime(new TimeSpan(0, 0, 0)),
                ref expr
            );
        }
        ...
        if (request.EndCheckInDate is not null)
        {
            AddCondition(
                x => x.CheckInDate.GetDateTime(x.CheckInTime)
                    .AddHourOffset(x.Facility!.TimeZone) <= request.EndCheckInDate.Value.GetDateTime(new TimeSpan(23, 59, 59)),
                ref expr
            );
        }
