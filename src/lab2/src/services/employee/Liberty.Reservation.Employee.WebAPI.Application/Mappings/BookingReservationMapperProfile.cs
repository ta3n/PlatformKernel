...
public class BookingReservationMapperProfile : BookingMapperProfile
{
    public BookingReservationMapperProfile()
    {
        CreateMap<Reservation.Application.Contexts.DataContexts.Entities.Data.Reservation, BookingReservationResponse>()
            ...
            .ForMember(
                dest => dest.FacilityTimeZoneId,
                opt => opt.MapFrom(
                    src => src.Facility!.TimeZoneId
                )
            )
            .ForMember(
                dest => dest.FacilityTimeZone,
                opt => opt.MapFrom(
                    src => src.Facility!.TimeZone
                )
            )
            .ForMember(
            ...
            .ForMember(
                ...
                opt => opt.MapFrom(
                    src => AppDate.GetId(
                        AppDate.GetDateTime(src.CheckInDate, src.CheckInTime)
                        .Add(DefaultValues.DefaultTimeZoneOffset - (src.Facility!.TimeZone ?? DefaultValues.DefaultTimeZoneOffset))
                    )
                )
            )
            .ForMember(
            ...
            .ForMember(
                dest => dest.CancellationPrice,
                opt => opt.MapFrom(
                    src => src.CancellationPrice
                )
            )
            .ForMember(
                dest => dest.RootCode,
        ...
        CreateMap<BookingReservationResponse, BookingReservationCsv>()
            ...
            .ForMember(
                dest => dest.CancellationRate,
                opt => opt.MapFrom(
                    src => src.CancellationRate
                )
            )
            .ForMember(
                dest => dest.CancellationPrice,
            ...
            .ForMember(
                dest => dest.CancellationPayment,
                opt => opt.MapFrom(
                    src => DefaultValues.ConvertCancellationPaymentTypeToJapanese(src.PaymentType ?? string.Empty)
                )
            );

        CreateMap<GmoPaymentResultRequest, GmoPaymentResultResponse>()
            .ForMember(
                dest => dest.ReservationCode,
                opt => opt.MapFrom(
                    src =>
                        src.OrderGmoPaymentResultRequests!.FirstOrDefault()!.Order
                            !.OrderReservations!.FirstOrDefault()!.Reservation!.Code
                )
            )
            .ForMember(
                ...
                opt => opt.MapFrom(
                    src =>
                        src.OrderGmoPaymentResultRequests!.FirstOrDefault()!.Order
                            !.OrderReservations!.FirstOrDefault()!.Reservation!.Id
                )
            );
        ...
        CreateMap<SaleDetailResponse, SaleDetailResponseCsv>()
            .ForMember(
                dest => dest.CheckInDate,
                opt => opt.MapFrom(
            ...
            .ForMember(
                dest => dest.CheckOutDate,
                opt => opt.MapFrom(
                    src => AppDate.GetDateTime(src.CheckOutDate, null)
                )
            )
            .ForMember(
                dest => dest.PaymentType,
