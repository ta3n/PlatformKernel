...
) : BaseMiddleware
{
    ...
    )
    {
        ...
        if (isFacilityAvailableOfMembership)
        {
            ...
            var (isFacilityAvailable, facilityId, facilityTimeZone) = await facilityRepository.CheckFacilityAvailableAsync(facilityCode);
            if (isFacilityAvailable)
            {
                ...
                context.Request.Headers.Append(
                    SecurityContextAccessor.FacilityHeaderTimeZone,
                    $"{facilityTimeZone}"
                );
