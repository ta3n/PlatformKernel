...
public partial class KakusanGetAllBookingQueryValidator : AbstractValidator<KakusanGetAllBookingQuery>
{
    ...
    )
    {
        try
        {
            ...
            if (dateNow.AddDays(-1 * _setting.MaxFromDays) > fromDay)
            {
                return false;
            }
            ...
            if (fromDay > request.GetToDay())
            {
                return false;
            }

            var rangeDays = Math.Round((request.GetToDay().Date - request.GetFromDay().Date).TotalDays) + 1;
