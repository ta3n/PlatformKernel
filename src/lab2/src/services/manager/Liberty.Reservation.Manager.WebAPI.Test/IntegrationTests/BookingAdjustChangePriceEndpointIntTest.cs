...
public class BookingAdjustChangePriceEndpointIntTest : BaseIntegrationTest
{
    private static string BaseUrl => "/api/reservations";
    private MockBookingData MockBookingData { get; }

    public BookingAdjustChangePriceEndpointIntTest()
    {
        MockBookingData = new MockBookingData(Factory, Client, FacilityInfo);
    }

    [Fact]
    public async Task AdjustBookingWithChangePrice_ReturnsOK()
    {
        ...
        await MockBookingData.CreatePlanAsync();
        await MockBookingData.CreateRoomGroupAsync();
        await MockBookingData.CreateSiteAsync();
    ...
    )
    {
        ...
        var checkInTime = DateTime.UtcNow.AddHours(DefaultValues.TimeZoneOffset).AddHours(0.5).ToString("HH\\:mm");
