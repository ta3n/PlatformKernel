...
public class PlanPriceEndpointUnitTest : BaseUnitTest
{
    protected override void InitData()
    {
        ...
        mockMediator.Setup(
                x => x.Send(
                    It.IsAny<PlanPriceGetRangePersonQuery>(),
                    It.IsAny<CancellationToken>()
                )
            )
            .ReturnsAsync(
                (
                    PlanPriceGetRangePersonQuery _,
                    CancellationToken _
                ) =>
                {
                    var response = new PlanRoomSiteRangePersonResponse(
                        1,
                        1,
                        1,
                        [
    ...
    [Fact]
    public async Task GetRangePerson_ReturnCorrectResult()
    {
        var plan = new Plan
        {
            Id = 1,
        ...
        // Act
        var result = await controller.GetRangePerson(plan.Id, 1, 1, planToken);
        var headers = ActionContext.HttpContext.Response.Headers;
        await result.ExecuteResultAsync(ActionContext);
        ...
        var actionResultWithHeaders = Assert.IsType<ActionResultWithHeaders>(result, false);
        var okObjectResult = Assert.IsType<OkObjectResult>(actionResultWithHeaders.Receiver, false);
        var response = Assert.IsType<PlanRoomSiteRangePersonResponse>(okObjectResult.Value, false);
