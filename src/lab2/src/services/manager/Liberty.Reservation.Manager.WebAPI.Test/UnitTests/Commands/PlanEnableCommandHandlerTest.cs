...
public class PlanEnableCommandHandlerTest
{
    [Fact]
    public async Task HandleAsync_ShouldReturnPlanId_WhenCommandValid()
    {
        ...
        mockPlanService.Setup(x => x.EnableAsync(It.IsAny<long>(), It.IsAny<bool>(), It.IsAny<bool>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(plan);

        var command = new PlanEnabledCommand(1) { Payload = new PlanEnabledRequest(true) };
        var handler = new PlanEnabledCommandHandler(
