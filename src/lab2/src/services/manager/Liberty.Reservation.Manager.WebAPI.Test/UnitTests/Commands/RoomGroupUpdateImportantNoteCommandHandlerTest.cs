...
public class RoomGroupUpdateImportantNoteCommandHandlerTest
{
    [Fact]
    public async Task HandleAsync_ShouldReturnPlanId_WhenCommandValid()
    {
        ...
        mockMediator.Setup(x => x.Send(It.IsAny<PlanUpdateImportantNoteCommand>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(planId);

        var command = new RoomGroupUpdateImportantNoteCommand(roomGroupId)
        {
            Payload = new RoomGroupUpdateImportantNoteRequest { Payment = "test" }
        };
        var handler = new RoomGroupUpdateImportantNoteCommandHandler(
