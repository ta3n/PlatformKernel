...
public class PlanPriceUpdateStandardPriceCommandHandlerTest
{
    ...
    public async Task HandleAsync_ShouldReturnPlanId_WhenCommandValid()
    {
        ...
        var mockPlanRoomGroupSitePriceDataService = new Mock<IPlanRoomGroupSitePriceDataService>();
        var facilityMinimumPriceData = new FacilityMinimumPriceData(false, 100);

        var mockServiceProvider = new Mock<IServiceProvider>();
        mockServiceProvider.Setup(sp => sp.GetService(typeof(IPlanRoomGroupSiteAppDateTypePriceService)))
            .Returns(mockPlanRoomGroupSiteAppdateTypePriceService.Object);
        mockServiceProvider.Setup(sp => sp.GetService(typeof(IAppDateTypeService)))
            .Returns(mockAppdateTypeService.Object);
        mockServiceProvider.Setup(sp => sp.GetService(typeof(IPlanService)))
        ...
        mockServiceProvider.Setup(sp => sp.GetService(typeof(IPlanRoomGroupSiteService)))
            .Returns(mockPlanRoomGroupSiteService.Object);
        mockServiceProvider.Setup(sp => sp.GetService(typeof(IPlanRoomGroupSitePriceDataService)))
            .Returns(mockPlanRoomGroupSitePriceDataService.Object);
