...
public class RoomGroupPriceUpdateSaleCommandHandlerTest
{
    [Fact]
    public async Task HandleAsync_ShouldReturnPlanId_WhenCommandValid()
    {
        ...
        mockMediator.Setup(x => x.Send(It.IsAny<PlanPriceUpdateSaleCommand>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(planId);

        var command = new RoomGroupPriceUpdateSaleCommand(roomTypeId, siteId)
        {
            Payload = new Application.Models.Requests.RoomGroupPriceUpdateSaleRequest()
        };

        var handler = new RoomGroupPriceUpdateSaleCommandHandler(
