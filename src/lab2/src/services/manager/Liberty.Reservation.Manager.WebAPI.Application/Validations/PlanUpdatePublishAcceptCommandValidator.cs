...
public class PlanUpdatePublishAcceptCommandValidator : AbstractValidator<PlanUpdatePublishAcceptCommand>
{
    public PlanUpdatePublishAcceptCommandValidator()
    {
        ...
        RuleFor(x => x.Payload)
            .Must(p =>
                p.ReceptionLimit.HasValue &&
                p.ReceptionDayLimit.HasValue
            )
            .WithErrorCode(ErrorCode.E2078)
            .WithMessage(ErrorCode.E2078.GetEnumDescriptions());
        ...
        RuleFor(x => x.Payload.ReceptionStartDayLimit)
            .Cascade(CascadeMode.Stop)
            .GreaterThanOrEqualTo(0)
            .WithErrorCode(ErrorCode.E1071)
            .WithMessage(ErrorCode.E1071.GetEnumDescriptions())
            .LessThanOrEqualTo(99)
            .WithErrorCode(ErrorCode.E1071)
            .WithMessage(ErrorCode.E1071.GetEnumDescriptions())
            .When(x => x.Payload.ReceptionStartDayLimit is not null);

        RuleFor(x => x.Payload.ReceptionStartLimit)
            .Cascade(CascadeMode.Stop)
            .Must(BeFullHour)
            .WithErrorCode(ErrorCode.E1072)
            .WithMessage(ErrorCode.E1072.GetEnumDescriptions())
            .When(x => x.Payload.ReceptionStartLimit is not null);
    }

    private static bool BeFullHour(
        TimeSpan? ts
    )
    {
        if (ts is null)
        ...
        return ts.Value.Minutes == 0 && ts.Value.Seconds == 0;
    }
