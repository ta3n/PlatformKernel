...
) : BaseEndpoint(mapper, mediator)
{
    ...
    {
        var addAppDateIds = await Mediator!.Send(
    ...
    [HttpGet("inventories")]
    [ProducesResponseType(typeof(List<RoomGroupWithAppDatesResponse>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetAllRoomGroupInventories(
        IPageable pageable,
        [FromQuery] long startAppDateId,
        [FromQuery] long endAppDateId,
        CancellationToken cancellationToken
