...
) : BaseEndpoint(mapper, mediator)
{
    ...
    [ProducesResponseType(typeof(BookingCancellationFeeResponse), StatusCodes.Status200OK)]
    ...
    [HttpGet("{id:long:min(1)}/audit-logs")]
    [ProducesResponseType(typeof(List<BookingAuditLogResponse>), StatusCodes.Status200OK)]
    public async Task<IActionResult> GetAllAuditLogOfReservation(
        [FromRoute] long id,
        CancellationToken cancellationToken
    )
    {
        var (headers, response) = await Mediator!.Send(
            new BookingGetAllHistoriesQuery(
                id,
                PageableConstants.UnPaged
            ),
            cancellationToken
