...
) : UpdateCommandWithAuditEventHandlerBase<BookingCancellationCommand, long>(unitOfWork, mapper, mediator)
{
    ...
    )
    {
        ...
        if (!existingReservation.CanModifyByUser())
        {
        ...
        var offset = existingReservation.Facility?.TimeZone ?? DefaultValues.DefaultTimeZoneOffset;

        var cancelledDateTime = DateTime.UtcNow.Add(offset);
