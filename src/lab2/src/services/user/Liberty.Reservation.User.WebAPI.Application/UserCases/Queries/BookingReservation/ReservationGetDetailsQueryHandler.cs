...
using Liberty.Reservation.User.WebAPI.Application.Models;
...
) : QuerySingleBaseHandler<ReservationGetDetailsQuery, ReservationDetailsResponse>(mapper)
{
    ...
    protected override async Task<(IHeaderDictionary, ReservationDetailsResponse)> HandleAsync(
    ...
    )
    {
        ...
        var query = BuildReservationQuery(request.Id, userCode);
    ...
    private ReservationDetailsResponse MapToReservationDetailResponse(
    ...
    )
    {
        ...
        var cancellationData = planData?.CancellationDataPolicy?.CancellationData ?? [];
        ...
        var (timeZoneId, timeZone) = bookingReservationService.GetFacilityTimeZoneInfoById(facilityData?.Id ?? 0);
        ...
        var isChange = reservation.CanModifyByUser(false);
        var isCancel = reservation.CanModifyByUser();
        ...
        var useDailyPerson = isChange && result.UseDailyPerson;
        var canAddRoomOnModify = isChange && result.CanAddRoomOnModify;
        var cancellationPrice = result.CancellationPrice;
        var bookingLanguageCode = $"{result.LanguageCode}";
        var barrierFreeInfoComment = DeserializeMultilingualText(result.BarrierFreeInfoComment, bookingLanguageCode);
        var spaTaxComment = DeserializeMultilingualText(result.SpaTaxComment, bookingLanguageCode);
        var spaTaxTable = DeserializeMultilingualText(result.SpaTaxTable, bookingLanguageCode);
        var payment = DeserializeMultilingualText(result.Payment, bookingLanguageCode);
        var meal = DeserializeMultilingualText(result.Meal, bookingLanguageCode);
        var other = DeserializeMultilingualText(result.Other, bookingLanguageCode);
        ...
        var cancellationTableSource = planData?.CancellationDataPolicy?.TableSource;
        var cancellationRuleDetail = planData?.CancellationDataPolicy?.RuleDetail;
        ...
        if (!string.IsNullOrWhiteSpace(result.CancellationDate)
            && DateTime.TryParse(
        ...
        {
            cancellationDate = parsed.ToUniversalTime();
        }
        ...
        {
            return json != null
                ? JsonConvert.DeserializeObject<MultilingualText>(json)?.GetValueByCode(languageCode)
                : string.Empty;
        }
    }
    ...
    )
    {
        ...
        query = FilterQuery(query, id, userCode);
    ...
    )
    {
        ...
        return query
            ...
            .SelectRaw(
                @$"{_reservationProp.FullColumnName(bookingDataName)}->>'{nameof(ReservationEntity.BookingData.MediaCode)}' AS ""{nameof(ReservationQueryResult.MediaCode)}"""
            )
            .SelectRaw(
    ...
    )
    {
        return query
            ...
            .Where(
                _reservationProp.FullColumnName(nameof(ReservationEntity.UserCode)),
                userCode
            )
