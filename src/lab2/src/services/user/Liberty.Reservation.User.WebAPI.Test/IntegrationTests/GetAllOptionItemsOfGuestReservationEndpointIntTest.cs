...
public class GetAllOptionItemsOfGuestReservationEndpointIntTest : BaseReservationEndpointIntTest
{
    private static new string BaseUrl => "api/guest/booking";

    [Fact]
    public async Task Guest_GetAllOptionItems_ReturnOK_WithOptionItemResponse()
    {
        ...
        _ = reservation.Id.ToString();
        _ = reservation.Code ?? string.Empty;
        _ = reservation.FacilityId.ToString();
        _ = reservation.FacilityId.ToString();
        _ = reservation.RoomGroupId.ToString();
        _ = reservation.SiteId.ToString();
        _ = (int)(AppDate.GetDateTime(reservation.CheckInDate).AddDays(1) - DateTime.UtcNow).TotalMinutes;

        var guestCode = GetGuestCode(reservation);

        var response = await Client.GetAsync($"{BaseUrl}/{guestCode}/option-items");
