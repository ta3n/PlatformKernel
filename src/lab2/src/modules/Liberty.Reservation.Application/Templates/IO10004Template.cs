...
public class Io10004Template(
    Io10004TemplateFormat format,
    SmtpMailSetting smtpMailSetting
) : BaseTemplate
{
    protected SmtpMailSetting Settings { get; set; } = smtpMailSetting;
    protected Io10004TemplateFormat Format { get; set; } = format;
    ...
    public override string Body
    {
        get
        {
            ...
            var args = new FormatDictionary
            {
                ...
                { "facilityName", data?.Facility.Name },
                { "facilityAddress", Reservation?.BookingData?.Facility.Address },
                { "facilityAccessInfo", data?.Facility.AccessInfoComment },
                ...
                {
                    "cancellationDaysBeforeFee", data?.Plan.CancellationDataPolicy?.TableSource?.GetValueByCode(
                        data.LanguageCode ?? LanguageHeaderUtil.GetLanguageCodeFromHeader()
                    )
                },
    ...
    public override List<MailTemplateParameter> GetMailTemplateParameters()
    {
        ...
        [
            ...
            MailParameter.FacilityAddressGoogleMap,
            MailParameter.UserUrl,
            MailParameter.MainUserTel,
            MailParameter.CancellationDaysBeforeFee,
            MailParameter.FacilityAccessInfo,
