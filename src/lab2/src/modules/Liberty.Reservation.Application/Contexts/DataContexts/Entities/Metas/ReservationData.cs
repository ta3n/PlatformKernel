...
public class ReservationData
{
    ...
    public List<ReservationUserData>? UserDatas()
    {
        return ReservationPlanRoomGroupAppDates?
            .Where(x => x.UserInfo is not null)
            .Select(x => x.UserInfo!)
            .ToList();
    }

    public List<ReservationPersonAgeTypeData>? PersonAgeTypes()
    {
        return ReservationPlanRoomGroupAppDates?
            .SelectMany(
    ...
    public List<ReservationRoomGroupAppDatePersonAgeTypeData>? ReservationPriceDatas()
    {
        return ReservationPlanRoomGroupAppDates?
            .SelectMany(x => x.ReservationRoomGroupAppDatePersonAgeTypes ?? [])
            .ToList();
    ...
    public List<ReservationRoomAppDateOptionItemData>? ReservationOptionItems()
    {
        return ReservationPlanRoomGroupAppDates?
            .SelectMany(x => x.ReservationRoomAppDateOptionItems ?? [])
            .ToList();
    }
    ...
    /// 部屋料
    /// </summary>
    public decimal? TotalRoomGroupPrice => ReservationPlanRoomGroupAppDates?
    ...
    /// 入湯税
    /// </summary>
    public decimal? TotalSpaTax => ReservationPlanRoomGroupAppDates?
        .SelectMany(a => a.ReservationRoomGroupAppDatePersonAgeTypes ?? [])
        .Sum(a => a.TotalSpaTax);
    ...
    /// オプションアイテム料
    /// </summary>
    public decimal? TotalOptionItemPrice => ReservationPlanRoomGroupAppDates?
        .SelectMany(a => a.ReservationRoomAppDateOptionItems ?? [])
        .Sum(a => a.TotalPrice);

    public decimal TotalPrice => TotalRoomGroupPrice ?? 0 + TotalSpaTax ?? 0 + TotalOptionItemPrice ?? 0;
    ...
    public ReservationSendMailState SendMailState { get; set; } = new();
}
...
public class ReservationSendMailState
{
