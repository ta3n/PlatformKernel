...
public class BookingReservationService(
    ...
    IBookingFacilityRepository bookingFacilityRepository
) : BaseService<ReservationEntity>(logger, reservationRepository, new ReservationNotfoundException()), IBookingReservationService
{
    public TimeSpan GetFacilityTimeZoneById(
        long facilityId
    )
    {
        var facilityTimeZone = bookingFacilityRepository
            .GetQueryableWithAsNoTracking()
            .Where(x => x.Id == facilityId)
            .Select(x => x.TimeZone ?? DefaultValues.DefaultTimeZoneOffset)
            .SingleOrDefault();

        return facilityTimeZone;
    }

    public (string TimeZoneId, TimeSpan Offset) GetFacilityTimeZoneInfoById(
        long facilityId
    )
    {
        var result = bookingFacilityRepository
            .GetQueryableWithAsNoTracking()
            .Where(x => x.Id == facilityId)
            .Select(
                x => new
        ...
        return (
            result?.TimeZoneId ?? DefaultValues.DefaultTimeZoneId,
            result?.TimeZone ?? DefaultValues.DefaultTimeZoneOffset
        );
    }
