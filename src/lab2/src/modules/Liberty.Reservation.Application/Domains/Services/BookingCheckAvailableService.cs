...
) : IBookingCheckAvailableService
{
    ...
    )
    {
        ...
        if (restNumber > DefaultValues.MaxRestNumber)
        {
            return false;
        }
    ...
    )
    {
        ...
        var existingPlanQueryable = bookingDataPlanRepo
            ...
            .Select(
                x => new
                {
                    ...
                    x.UseReceptionStartDay,
                    x.ReceptionStartDayLimit,
                    x.ReceptionStartLimit,
                    FacilityTimeZone = x.FacilityPlans!.FirstOrDefault()!.Facility!.TimeZone ?? DefaultValues.DefaultTimeZoneOffset
                }
            );
        ...
        var now = DateTime.UtcNow.Add(existingPlan.FacilityTimeZone);

        if (
            existingPlan.UseReceptionStartDay is not null
            && existingPlan.ReceptionStartDayLimit is not null
            && existingPlan.ReceptionStartLimit is not null
        )
        {
            var receptionStartDateTime = checkInDateTime
                .AddDays(-existingPlan.ReceptionStartDayLimit.Value)
                .Add(existingPlan.ReceptionStartLimit.Value);

            if (now < receptionStartDateTime)
            {
                return false;
            }
        }
    ...
    )
    {
        ...
        if (reservationId <= 0)
        {
            return HasInvalidNewOptions(listSearchOptionItem, datesOfOptionItems);
        }
    ...
    private static bool HasInvalidNewOptions(
        IEnumerable<OptionOfBookingPriceRequest> optionItems,
        OptionItemAppDate[] datesOfOptionItems
    )
    {
        return optionItems
            .GroupBy(
            ...
            .Any(
                group =>
                {
                    ...
                    var match = Array.Find(
                        datesOfOptionItems,
                        x => x.AppDateId == group.Key.AppDateId && x.OptionItemId == group.Key.OptionItemId
                    );

                    if (match is null || totalNumber > match.RemainNumber)
                    {
                        return true;
                    }

                    var maxSupply = match.OptionItem?.MaxSupplyNumber;

                    return maxSupply is not null && group.Any(x => x.Number > maxSupply);
                }
            );
    }
    ...
    public void IsQuestionsRequiredValidAll(
        IEnumerable<QuestionOfBookingCreateRequest> bookingQuestionCreate,
        IEnumerable<Question> existingQuestion
    )
    {
        existingQuestion = existingQuestion.Where(x => x.IsRequired).ToList();
        foreach (var item in bookingQuestionCreate)
        {
            var question = existingQuestion.FirstOrDefault(x => x.Id == item.QuestionId);
            if (question is not null && string.IsNullOrEmpty(item.AnswerData))
            {
                throw new ReservationNeedQuestionRequiredException(
                    question.Name?.GetValueByHeader() ?? ""
                );
            }
        }
    }
    ...
    public void IsQuestionsRequiredValidAll(
        IEnumerable<QuestionOfBookingCreateRequest> bookingQuestionCreate,
        IEnumerable<QuestionData> existingQuestion
    )
    {
        existingQuestion = existingQuestion.Where(x => x.IsRequired ?? true).ToList();
        foreach (var item in bookingQuestionCreate)
        {
            var question = existingQuestion.FirstOrDefault(x => x.Id == item.QuestionId);
            if (question is not null && string.IsNullOrEmpty(item.AnswerData))
