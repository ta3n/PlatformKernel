...
) : IBookingCreateService
{
    ...
    private readonly IBookingQuestionRepository _bookingQuestionRepository =
       serviceProvider.GetRequiredService<IBookingQuestionRepository>();
    ...
    )
    {
        ...
        {
            var bookingData = existingReservation.BookingData!;
            selectedQuestions =
            [
                .. bookingData.GetAllQuestions()
                    .Select(
                        x => new Question
                        {
                            Id = x.Id ?? 0,
                            Name = new MultilingualText
                                {
                                    { bookingData.LanguageCode ?? DefaultValues.LanguageCode, x.Name ?? string.Empty }
                                },
                            Description = new MultilingualText
                            {
                                { bookingData.LanguageCode ?? DefaultValues.LanguageCode, x.Description ?? string.Empty }
                            },
                            FormData = new MultilingualText
                            {
                                { bookingData.LanguageCode ?? DefaultValues.LanguageCode, x.FormData ?? string.Empty }
                            }
                        }
                    )
            ];
        ...
        var newIds = bookingCreateRequest.OptionsQuestions?
            .Select(x => x.QuestionId)
            .ToList() ?? [];

        var oldIds = selectedQuestions?
            .Select(x => x.Id)
            .ToList() ?? [];

        var idsNotInOld = newIds.Except(oldIds).ToList();

        var questionIdsToAdd = bookingCreateRequest.OptionsQuestions!
            .Where(x => idsNotInOld.Contains(x.QuestionId))
            .Select(x => x.QuestionId)
            .ToList();

        if (questionIdsToAdd.Count > 0)
        {
            var questionsToAdd = await _bookingQuestionRepository
                .GetQueryableWithAsNoTracking()
                .Where(x => questionIdsToAdd!.Contains(x.Id))
                .ToListAsync(cancellationToken);

            selectedQuestions!.AddRange(questionsToAdd);
        }
        ...
        if (bookingCreateRequest.PlanQuestions is not null)
        {
            foreach (var bookingQuestion in bookingCreateRequest.PlanQuestions)
            {
                var planQuestion = new ReservationQuestion
                {
                    ...
                    IsEnabled = true,
                    IsRequired = bookingQuestion.IsRequired
                };
    ...
    )
    {
        ...
        var destinationPrefixName = newReservation.BookingData.Site.PrefixName;

        newReservation.NoticeNumber = destinationPrefixName is null
            ? null
            : EntityUtil.CreateNoticeNumber(destinationPrefixName);
    ...
    )
    {
        ...
        return
        [
            .. reservationOptionItemData.Select(
                optionItemData => new ReservationRoomGroupAppDateOptionItem
                {
                    Reservation = newReservation,
                    RoomGroupId = newReservation.RoomGroupId,
                    BookingDateId = appDateId,
                    OptionItemId = optionItemData.OptionItemInfo?.Id ?? 0,
                    OptionItem = new OptionItem { Name = optionItemData.OptionItemInfo?.Name },
                    RestIndex = nightIndex,
                    RoomGroupIndex = roomIndex,
