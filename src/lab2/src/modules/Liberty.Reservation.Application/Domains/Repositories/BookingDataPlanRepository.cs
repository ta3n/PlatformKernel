        ...
        IQueryable<BookingPlanModel> GetQueryableBookingPlan()
        {
            var queryable = GetQueryableWithAsNoTracking()
                ...
                .Where(
                    plan =>
                        (
                            plan.IsOnLinePayment
                            && facilityState.CanOnLinePayment
                            && facilityState.IsOnLinePayment
                            && systemCanOnlinePayment
                        )
                        || (
                ...
                .Select(
                    plan => new BookingPlanModel(
                        ...
                        plan.UseReceptionStartDay,
                        plan.ReceptionStartDayLimit,
                        plan.ReceptionStartLimit
                    ) { Code = plan.Code }
                )
                .AsSingleQuery();
...
public static class BookingSearchQueryableExtensions
{
    ...
    )
    {
        return queryable
            ...
            .Where(x => !x.UseBookingReception || x.BookingReceptionEnd == null || x.BookingReceptionEnd >= bookingSearchDateId)
