...
) : IBookingDataPriceRepository
{
    ...
    )
    {
        ...
        var rangePersonData = await GetAllRangePersonModelsAsync(
            facilityId,
            siteId,
            planIds,
            roomGroupIds,
        ...
        var standardPriceLookup = new HashSet<(int?, int?)>(
            rangePersonData.Select(t => (t.PersonMin, t.PersonMax))
        );
    ...
    )
    {
        ...
        var standardPriceLookup = new HashSet<(int?, int?, long, long)>(
            rangePersonData.Select(t => (t.PersonMin, t.PersonMax, t.RoomGroupId, t.PlanId))
        );
    ...
    )
    {
        var cacheKeys = (
            ...
            from roomGroupId in roomGroupIds
            let cacheKey = string.Format(
                CacheKeys.BookingSearchRangePersonPrefixKey,
                facilityId,
                siteId,
                planId,
                roomGroupId
        ...
        var cachedData = new List<BookingMetaRangePersonModel>();
        if (useCache)
        {
            cachedData =
            [
                .. cacheService.GetByPatterns<BookingMetaRangePersonModel>(
                        [.. cacheKeys.Select(x => x.cachePattern).Distinct()]
                    )
                    .Where(
                        x => cacheKeys.Exists(
        ...
        await using var dbContext = await dbContextFactory.CreateDbContextAsync(cancellationToken);
        var queryable = dbContext.Set<PlanRoomGroupSitePriceData>()
            .AsNoTracking()
            .Where(
                planRoomGroupSitePriceData =>
                    planRoomGroupSitePriceData.SiteId == siteId
            )
            .Where(
                planRoomGroupSitePriceData =>
                    missingPlanIds.Contains(planRoomGroupSitePriceData.PlanId)
            )
            .Where(
                planRoomGroupSitePriceData =>
                    missingRoomGroupIds.Contains(planRoomGroupSitePriceData.RoomGroupId)
            )
            .Where(x => x.IsEnabled)
            .OrderBy(
                planRoomGroupSitePriceData => planRoomGroupSitePriceData.PersonMin
            )
            .Select(
                planRoomGroupSiteAppDateTypePriceData => new BookingMetaRangePersonModel(
                    planRoomGroupSiteAppDateTypePriceData.PlanId,
                    planRoomGroupSiteAppDateTypePriceData.RoomGroupId,
                    planRoomGroupSiteAppDateTypePriceData.PersonMin,
                    planRoomGroupSiteAppDateTypePriceData.PersonMax
                )
            )
            .AsSingleQuery();
        ...
        var mergedData = rangePersons.Union(cachedData).ToList();

        var bulkCacheData = new Dictionary<string, List<BookingMetaRangePersonModel>>();
