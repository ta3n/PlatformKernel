...
public static class EntityPropertyExtension
{
    ...
    /// </returns>
    /// <exception cref="AppLibertyException">
        ...
        where T : IBaseEntity
    {
        var entityType = context
            ...
            ?? throw new AppLibertyException("Entity type is not null");

        var properties = entityType.GetProperties();
        var schema = entityType.GetSchema() ?? throw new AppLibertyException("Schema of the entity is not null");
        var tableName = entityType.GetTableName() ?? throw new AppLibertyException("Table name of the entity is not null");
        ...
        foreach (var property in properties)
        {
            ...
            var columnName = property.GetColumnName(StoreObjectIdentifier.Table(tableName, schema))
                ?? throw new AppLibertyException($"{fieldName} of the {nameof(T)} not get mapping column name.");
...
public class EntityProperty
{
    ...
    /// <exception cref="Liberty.SysException.Exceptions.AppLibertyException">
