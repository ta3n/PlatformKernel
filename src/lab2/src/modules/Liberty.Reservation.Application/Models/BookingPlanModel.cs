...
public record BookingPlanModel(
    ...
    bool? UseReceptionStartDay,
    int? ReceptionStartDayLimit,
    TimeSpan? ReceptionStartLimit
)
{
    ...
    /// </param>
    /// <param name="appDateId">The check-in date to check.</param>
    /// <returns>
    ...
    {
        var checkInDateTime = AppDate.GetDateTime(appDateId);
        var now = DateTime.UtcNow.Add(facilityTimeZone);

        if (
            UseReceptionStartDay is not null
            && ReceptionStartDayLimit is not null
            && ReceptionStartLimit is not null
            && (isSearchPrice || appDateId == checkInDate)
        )
        {
            var receptionStartDateTime = checkInDateTime
                .AddDays(-ReceptionStartDayLimit.Value)
                .Add(ReceptionStartLimit.Value);
        ...
        var isAvailable = BookingReceptionStart <= appDateId
            && BookingReceptionEnd >= appDateId;
