...
public sealed class TimescaleDemoRunner
{
    ...
    )
    {
        ...
        var timescaleVersion = await connection.ExecuteScalarAsync<string>(new CommandDefinition(
            """
            SELECT extversion
            FROM pg_extension
            WHERE extname = 'timescaledb';
            """,
            cancellationToken: cancellationToken)) ?? "unknown";

        var toolkitInstalled = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
                FROM pg_extension
        ...
        var modernHypertableExists = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
        ...
        var seededRowCount = await connection.ExecuteScalarAsync<int>(new CommandDefinition(
            "SELECT COUNT(*) FROM metrics;",
            cancellationToken: cancellationToken));

        var chunkCount = await connection.ExecuteScalarAsync<int>(new CommandDefinition(
            """
            SELECT COUNT(*)
        ...
        var hasContinuousAggregatePolicy = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
        ...
        var hasRetentionPolicy = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
                FROM timescaledb_information.jobs
        ...
        var hasColumnstorePolicy = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
                FROM timescaledb_information.jobs
        ...
        var hypertables = (await connection.QueryAsync<string>(new CommandDefinition(
            """
        ...
        var bucketedMetrics = (await connection.QueryAsync<BucketedMetric>(new CommandDefinition(
            """
            SELECT
        ...
        var hourlyAggregates = (await connection.QueryAsync<HourlyMetricAggregate>(new CommandDefinition(
            """
            SELECT
                bucket AS Bucket,
                device_id AS DeviceId,
        ...
        var jobs = (await connection.QueryAsync<TimescaleJobInfo>(new CommandDefinition(
            """
            SELECT
                job_id AS JobId,
    ...
    )
    {
        ...
        var isHypertable = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
                FROM timescaledb_information.hypertables
        ...
        var rowCount = await connection.ExecuteScalarAsync<int>(new CommandDefinition(
    ...
    )
    {
        ...
        var functionAvailable = await connection.ExecuteScalarAsync<bool>(new CommandDefinition(
            """
            SELECT EXISTS (
                SELECT 1
                FROM pg_proc
