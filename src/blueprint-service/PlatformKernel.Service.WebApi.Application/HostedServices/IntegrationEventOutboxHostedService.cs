...
using PlatformKernel.Hangfire;
using PlatformKernel.Hangfire.Models;
using PlatformKernel.Service.WebApi.Application.Settings;
using PlatformKernel.Service.WebApi.Application.Web.ApiService;

namespace PlatformKernel.Service.WebApi.Application.HostedServices;
    ...
    )
{
    ...
    )
    {
        ...
        try
        {
            ...
            //     string.Join(", ", eventsToRemove.Select<IntegrationEventOutbox, object>(x => x.Id))
            // );
        }
        catch (Exception ex)
        {
