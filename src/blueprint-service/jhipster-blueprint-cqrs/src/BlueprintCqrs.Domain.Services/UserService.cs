...
using BlueprintCqrs.Crosscutting.Utilities;
...
) : IUserService
{
    ...
    public virtual async Task<User> CreateUser(
        User userToCreate
    )
    {
        var user = new User
    ...
    public async Task<IPage<User>> GetAllManagedUsers(
        IPageable pageable
    )
    {
    ...
    public async Task<IPage<User>> GetAllPublicUsers(
        IPageable pageable
    )
    {
        return await _userManager.Users
    ...
    public async Task<User> GetByLogin(
        string login
    )
    {
        return await _userManager.Users
    ...
    public virtual async Task<User> UpdateUser(
        User userToUpdate
    )
    {
        //TODO use Optional
    ...
    public virtual async Task<User> CompletePasswordReset(
        string newPassword,
        string key
    )
    {
        ...
        if (user == null || user.ResetDate <= DateTime.Now.Subtract(86400.Seconds()))
        {
            return null;
        }
    ...
    public virtual async Task<User> RequestPasswordReset(
        string mail
    )
    {
    ...
    public virtual async Task<User> ActivateRegistration(
        string key
    )
    {
    ...
    public virtual async Task<User> RegisterUser(
        User userToRegister,
        string password
    )
    {
    ...
    private async Task<User> GetUserWithUserRolesByName(
        string name
    )
    {
        return await _userManager.Users
    ...
    private async Task<bool> RemoveNonActivatedUser(
        User existingUser
    )
    {
