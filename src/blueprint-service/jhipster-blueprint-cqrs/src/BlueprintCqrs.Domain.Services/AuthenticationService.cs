...
{
    private readonly ILogger<AuthenticationService> _log = log;

    private readonly UserManager<User> _userManager = userManager;

    public virtual async Task<IPrincipal> Authenticate(
        string username,
        string password
    )
    {
        ...
        if (!user.Activated)
        {
            throw new UserNotActivatedException($"User {user.UserName} was not activated.");
        }
    ...
    private async Task<User> LoadUserByUsername(
        string username
    )
    {
    ...
    private async Task<IPrincipal> CreatePrincipal(
        User user
    )
    {
