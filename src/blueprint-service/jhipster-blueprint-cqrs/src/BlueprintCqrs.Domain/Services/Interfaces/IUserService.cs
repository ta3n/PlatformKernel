...
public interface IUserService
{
    ...
    Task<IPage<User>> GetAllManagedUsers(
        IPageable pageable
    );

    Task<User> GetByLogin(
        string login
    );
    ...
    Task<User> CompletePasswordReset(
        string newPassword,
        string key
    );

    Task<User> RequestPasswordReset(
        string mail
    );
    ...
    Task<User> ActivateRegistration(
        string key
    );
    ...
    Task<IPage<User>> GetAllPublicUsers(
        IPageable pageable
    );
}
