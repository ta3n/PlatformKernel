...
{
	Task<Result<CreateStockResponse>> HandleAsync(
	...
	: ICreateStockHandler
{
	...
	)
	{
		...
		logger.LogInformation(
			"Created stock for product '{ProductName}' with quantity {Quantity}",
			request.ProductName,
			request.Quantity
		);
	...
	private static CreateStockResponse MapToCreateStockResponse(
		ProductStock stock
	)
	{
		return new CreateStockResponse(stock.ProductName, stock.AvailableQuantity);
	}
}
