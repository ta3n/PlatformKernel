...
{
	Task<Result<StockResponse>> HandleAsync(
	...
	: IGetStocksByProductNameHandler
{
	...
	)
	{
		...
		logger.LogInformation(
			"Retrieved stock for product '{ProductName}': {Quantity}",
			request.ProductName,
	...
	private static StockResponse MapToStockResponse(
		Domain.Entities.ProductStock stock
	)
	{
		return new StockResponse(stock.ProductName, stock.AvailableQuantity);
	}
}
