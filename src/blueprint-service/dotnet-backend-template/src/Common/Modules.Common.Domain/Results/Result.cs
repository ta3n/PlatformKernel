...
public readonly partial record struct Result<TValue> : IResult<TValue>
{
		...
		get
		{
			if (IsError)
			{
				throw new InvalidOperationException(
					"The Value property cannot be accessed when Errors property is not empty. Check IsSuccess or IsError before accessing the Value."
				);
			}
	...
	public Error FirstError => !IsError
		? throw new InvalidOperationException(
			"The FirstError property cannot be accessed when Errors property is empty. Check IsError before accessing FirstError."
		)
		: Errors[0];
}
