	...
	: IEventHandler<ShipmentCreatedEvent>
{
	...
	)
	{
		...
		try
		{
			...
			{
				logger.LogError(
					"Failed to update stock for order {OrderId}: {@Errors}",
	...
	private static DecreaseStockRequest CreateDecreaseStockRequest(
		Shipment shipment
	)
	{
		return new DecreaseStockRequest(
			[.. shipment.Items.Select(x => new ProductStock(x.Product, x.Quantity))]
		);
	}
}
