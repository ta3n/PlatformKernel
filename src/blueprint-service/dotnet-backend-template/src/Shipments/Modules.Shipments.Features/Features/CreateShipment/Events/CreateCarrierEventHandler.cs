	...
	: IEventHandler<ShipmentCreatedEvent>
{
	...
	)
	{
		...
		try
		{
			...
			{
				logger.LogError(
					"Failed to create carrier shipment for order {OrderId}: {@Errors}",
	...
	private static CreateCarrierShipmentRequest CreateCarrierRequest(
		Shipment shipment
	)
	{
		return new CreateCarrierShipmentRequest(
			...
			[.. shipment.Items.Select(x => new CarrierShipmentItem(x.Product, x.Quantity))]
		);
	}
}
