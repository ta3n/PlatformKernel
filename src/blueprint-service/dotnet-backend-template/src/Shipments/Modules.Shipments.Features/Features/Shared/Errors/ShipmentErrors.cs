...
internal static class ShipmentErrors
{
	...
	{
		return Error.NotFound($"{ErrorPrefix}.{nameof(NotFound)}", $"Shipment with number '{shipmentNumber}' not found");
	}
	...
	{
		return Error.Conflict($"{ErrorPrefix}.{nameof(AlreadyExists)}", $"Shipment for order '{orderId}' already exists");
	}
}
