# Detail Design - Reservation - Member - Online Payment

------------------------------------------------------------------

## 1. Activity Diagram

### 1.1 Activity Diagram for Create A Reservation

```mermaid
flowchart TD
  A((Start)) -- Request --> B["`
    Call BookingEndpoint
  ...
  T --> U[Call IBookingCreateService.GetQuestions]
  U --> V[Call IBookingCreateService.GetReservationDataAsync]
  V --> W[Call IBookingCreateService.GetDataAvailableAsync]
  W --> X[Call IBookingCreateService.GetAllRoomAppDates]
  X --> Y[Call IBookingCreateService.GetAllReservationPriceData]
  Y --> Z[Call IBookingCreateService.GetAllReservationOptionItemData]
  Z --> AA[Call IBookingCreateService.CreateReservationPlanRoomGroupAppDate]
  AA --> AB[Call IBookingCreateService.GetReservationPriceData]
  AB --> AC[Call IBookingCreateService.GetReservationOptionItemData]
  AC --> AD[Call IBookingCreateService.GetBookingData]
  AD --> AE[Call IOrderBookingService.CreateAsync]
  AE --> AF{If there is an error create Order & Reservation?}
  AF -- Yes --> AG[Call IUnitOfWork.RollbackAsync]
  AG --> AH[Throw AppLibertyException]
  AF -- No --> AJ
  G --> AJ[Return Response]
  J --> AJ
  P --> AJ
  S --> AJ
  M --> AJ
...
> **Note**: The `Activity Diagram for Create A Reservation` describes the sequence of activities and interactions between components in the
> process of creating booking a member (user login).
> The **`BookingEndPoint`** have route request parameters **`planId`**, **`roomGroupId`** and request payload **`SiteBookingCreateRequest`
**.
> End of `Activity Diagram for Create A Reservation` return response the booking code.

### 1.1.1 Request (SiteBookingCreateRequest)
...
### 1.1.2 Response (Booking Code)

The `BookingEndPoint` response to `Client` the booking code of a reservation created.

### 1.2 Activity Diagram for GMO Online Payment Result

```mermaid
flowchart TD
  A((Start)) -- Request --> B["Call ReservationsEndpoint /api/reservations/payment-result"]
  B --> C[Call BookingGmoPaymentCommandHandler]
  C --> D[IBookingCheckAvailableService.GetReservationByUserAsync]
  D --> E{Is Reservation Exists?}
  E -- Yes --> G{Is Payment Online Reservation Valid?}
  E -- No --> F[Throw ReservationNotfoundException]
  G -- Yes --> J[IOrderBookingService.GetOrderIdByReservationId]
  G -- No --> H[Throw ReservationInvalidException]
  J --> K[IBookingCheckAvailableService.IsNightNumber]
  K --> L[IBookingCheckAvailableService.IsRoomNumber]
  L --> M[Check Night Number Availability]
  M --> N[Check Room Number Availability]
  N --> O{If Booking Not Available and GMO Successfull}
  O -- No --> O1[IOrderGmoPaymentService.CreateAsync]
  O1 --> Q[IBookingReservationService.ConfirmedAsync]
  O -- Yes --> P[Call OnlinePaymentRefundAsync]
  P --> Q1[Call IBookingReservationService.FindOrderIdOfOnlinePaymentAsync]
  Q1 --> R1[Call IOrderGmoPaymentService.SearchTradeAsync]
  R1 --> S1[Call IOrderGmoPaymentService.CancelAsync]
  S1 --> P1{Is Refund Successfully?}
  P1 -- Yes --> P2[Return Refund Successfully Message]
  P1 -- No --> P3[Return Refund Error Message]
  Q --> U{Successfully?}
  U -- Yes --> R[SendMail]
  U -- No --> W[Rollback Transaction]
  R --> S[RegisterScheduleJobAsync]
  S --> T[Return Success Response]
  W --> X[Throw AppLibertyException]
  F --> Y((Stop))
  H --> Y((Stop))
  P2 --> Y((Stop))
  P3 --> Y((Stop))
...
> **Note**: The `Activity Diagram for GMO Online Payment Result` describes the sequence of activities and interactions between components in
> the process of handling an online payment request in the `BookingGmoPaymentCommandHandler`.
> The **`ReservationsEndpoint`** receives the request and forwards it to the **`BookingGmoPaymentCommandHandler`**.
> The **`BookingGmoPaymentCommandHandler`** performs various checks and processes the payment, including handling refunds if the booking is
> not available.
> End of `Activity Diagram for GMO Online Payment Result` returns the final response to the client.

### 1.2.1 GmoPaymentRequest

| No | Parameter  | Description                                               |
|----|------------|-----------------------------------------------------------|
| 1  | ShopId     | The ID of the shop (optional).                            |
| 2  | ShopPass   | The password of the shop (optional).                      |
| 3  | AccessId   | The access ID (optional).                                 |
| 4  | AccessPass | The access password (optional).                           |
| 5  | OrderId    | The order ID (optional).                                  |
| 6  | Status     | The status of the payment (optional).                     |
| 7  | JobCd      | The job code (optional).                                  |
| 8  | Amount     | The amount of the payment (optional).                     |
| 9  | Tax        | The tax amount (optional).                                |
| 10 | Currency   | The currency of the payment (optional).                   |
| 11 | Forward    | The forward code (optional).                              |
| 12 | Method     | The payment method (optional).                            |
| 13 | PayTimes   | The number of payment times (optional).                   |
| 14 | TranId     | The transaction ID (optional).                            |
| 15 | Approve    | The approval code (optional).                             |
| 16 | TranDate   | The transaction date (optional).                          |
| 17 | ErrCode    | The error code (optional).                                |
| 18 | ErrInfo    | The error information (optional).                         |
| 19 | PayType    | The type of payment (optional).                           |
| 20 | Id         | The unique identifier for the payment request (required). |

> **Note**: The `Id` parameter is required and must be provided. All other parameters are optional and can be null.

### 1.2.2 Response (GmoPayment Result Id)

The `ReservationsEndpoint` response to `Client` the GMO payment result id of a OrderGmoPaymentResult created.

## 2. Sequence Diagram

## 2.1 Sequence Diagram for Create A Reservation
...
sequenceDiagram
  ...
  Application.BookingCreateCommandHandler ->> Client: 13.4 Throw AppLibertyException
...
### Description Sequence Diagram for Create A Reservation
...
| 13.4   | Application.BookingCreateCommandHandler → Client                           | If database error `Rollback` && return error to `Client`.                                 |
...
## 2.2 Sequence Diagram for GMO Online Payment Result

```mermaid
sequenceDiagram
  participant Client
  participant ReservationsEndpoint
  participant BookingGmoPaymentCommandHandler
  participant IBookingCheckAvailableService
  participant IOrderBookingService
  participant IOrderGmoPaymentService
  ...
  participant IRepository
  participant Database
  participant Reservation.Mail.Worker.Application
  Client ->> ReservationsEndpoint: 1. Request Payload (GmoPaymentRequest)
  ReservationsEndpoint ->> BookingGmoPaymentCommandHandler: 2. Send Payment Command (payload)
  BookingGmoPaymentCommandHandler ->> IBookingCheckAvailableService: 3. GetReservationByUserAsync (userCode, cancellationToken)
  ...
  IBookingCheckAvailableService -->> BookingGmoPaymentCommandHandler: 3.3 Return reservation data
  BookingGmoPaymentCommandHandler ->> BookingGmoPaymentCommandHandler: 4. Check Reservation Validity
  alt Reservation Not Found
    BookingGmoPaymentCommandHandler ->> Client: 5.1 Throw ReservationNotfoundException
  end
  BookingGmoPaymentCommandHandler ->> BookingGmoPaymentCommandHandler: 5.2 Check IsOnlinePayment & Reservation Stauts Valid
  alt Reservation Invalid
    BookingGmoPaymentCommandHandler ->> Client: 5.3 Throw ReservationInvalidException
  end
  BookingGmoPaymentCommandHandler ->> IOrderBookingService: 6. GetOrderIdByReservationId (reservationId, cancellationToken)
  IOrderBookingService ->> IRepository: 6.1 Query order data
  IRepository ->> Database: 6.1.1 Query data
  Database -->> IRepository: 6.1.2 Return data
  IRepository -->> IOrderBookingService: 6.2 Return order data
  IOrderBookingService -->> BookingGmoPaymentCommandHandler: 6.3 Return order ID
  BookingGmoPaymentCommandHandler ->> IBookingCheckAvailableService: 7. IsNightNumber (planId, roomGroupId, siteId, checkInDate, restNumber, cancellationToken)
  IBookingCheckAvailableService ->> IRepository: 7.1 Query night number data
  IRepository ->> Database: 7.1.1 Query data
  Database -->> IRepository: 7.1.2 Return data
  IRepository -->> IBookingCheckAvailableService: 7.2 Return night number data
  IBookingCheckAvailableService -->> BookingGmoPaymentCommandHandler: 7.3 Return night number availability
  BookingGmoPaymentCommandHandler ->> IBookingCheckAvailableService: 8. IsRoomNumber (planId, roomGroupId, checkInDate, roomNumber, cancellationToken)
  IBookingCheckAvailableService ->> IRepository: 8.1 Query room number data
  IRepository ->> Database: 8.1.1 Query data
  Database -->> IRepository: 8.1.2 Return data
  IRepository -->> IBookingCheckAvailableService: 8.2 Return room number data
  IBookingCheckAvailableService -->> BookingGmoPaymentCommandHandler: 8.3 Return room number availability
  BookingGmoPaymentCommandHandler ->> BookingGmoPaymentCommandHandler: 9. Check Booking Availability
  alt Booking Not Available
    BookingGmoPaymentCommandHandler ->> IOrderGmoPaymentService: 9.1 Call OnlinePaymentRefundAsync
    IOrderGmoPaymentService ->> IBookingReservationService: 9.1.1 Call FindOrderIdOfOnlinePaymentAsync
    IBookingReservationService ->> IGmoPaymentGatewayService: 9.1.2 Call SearchTradeAsync
    IGmoPaymentGatewayService -->> IBookingReservationService: 9.1.3 Return Trade Data
    IBookingReservationService ->> IGmoPaymentGatewayService: 9.1.4 Call CancelAsync
    IGmoPaymentGatewayService -->> IBookingReservationService: 9.1.5 Return Cancel Data
    alt Refund Successful
      IGmoPaymentGatewayService -->> BookingGmoPaymentCommandHandler: 9.1.6 Return Refund Successful Message
    else Refund Error
      IGmoPaymentGatewayService -->> BookingGmoPaymentCommandHandler: 9.1.7 Return Refund Error Message
    end
  end
  BookingGmoPaymentCommandHandler ->> IOrderGmoPaymentService: 10. CreateAsync
  IOrderGmoPaymentService ->> IBookingReservationService: 10.1 ConfirmedAsync
  BookingGmoPaymentCommandHandler ->> BookingGmoPaymentCommandHandler: 11. GMO online payment result
  alt GMO Online Payment Result Error
    BookingGmoPaymentCommandHandler ->> BookingGmoPaymentCommandHandler: 12. Log Error
    BookingGmoPaymentCommandHandler ->> BookingGmoPaymentCommandHandler: 13. Rollback Transaction
    BookingGmoPaymentCommandHandler ->> Client: 14. Throw AppLibertyException
  end

  BookingGmoPaymentCommandHandler ->> MailJobScheduler: 15 Register Send Mail Schedule Job
  MailJobScheduler ->> Reservation.MailService: 15.1 Publish to queue RabbitMQ
  MailJobScheduler -->> BookingGmoPaymentCommandHandler: 15.2 Return response
  BookingGmoPaymentCommandHandler -->> ReservationsEndpoint: 16. Return Success Response
  ReservationsEndpoint -->> Client: 17. Return Response
```

### Description for Sequence Diagram for Create A Reservation

| No.   | Activity                                                        | Description                                                                            |
|-------|-----------------------------------------------------------------|----------------------------------------------------------------------------------------|
| 1     | Client → ReservationsEndpoint                                   | The `Client` sends a `GmoPaymentRequest` payload to the `ReservationsEndpoint`.        |
| 2     | ReservationsEndpoint → BookingGmoPaymentCommandHandler          | `ReservationsEndpoint` forwards the request to `BookingGmoPaymentCommandHandler`.      |
| 3     | BookingGmoPaymentCommandHandler → IBookingCheckAvailableService | Calls `GetReservationByUserAsync` to get the reservation by user.                      |
...
| 3.3   | IBookingCheckAvailableService → BookingGmoPaymentCommandHandler | Returns the reservation data to `BookingGmoPaymentCommandHandler`.                     |
| 4     | BookingGmoPaymentCommandHandler                                 | Checks the reservation validity.                                                       |
| 5.1   | BookingGmoPaymentCommandHandler → Client                        | Throws `ReservationNotfoundException` if the reservation is not found.                 |
| 5.2   | BookingGmoPaymentCommandHandler                                 | Checks if the reservation is an online payment and if the reservation status is valid. |
| 5.3   | BookingGmoPaymentCommandHandler → Client                        | Throws `ReservationInvalidException` if the reservation is invalid.                    |
| 6     | BookingGmoPaymentCommandHandler → IOrderBookingService          | Calls `GetOrderIdByReservationId` to get the order ID by reservation ID.               |
| 6.1   | IOrderBookingService → IRepository                              | Queries the order data from `IRepository`.                                             |
| 6.1.1 | IRepository → Database                                          | Queries the data from `Database`.                                                      |
| 6.1.2 | Database → IRepository                                          | Returns the data to `IRepository`.                                                     |
| 6.2   | IRepository → IOrderBookingService                              | Returns the order data to `IOrderBookingService`.                                      |
| 6.3   | IOrderBookingService → BookingGmoPaymentCommandHandler          | Returns the order ID to `BookingGmoPaymentCommandHandler`.                             |
| 7     | BookingGmoPaymentCommandHandler → IBookingCheckAvailableService | Calls `IsNightNumber` to check night number availability.                              |
| 7.1   | IBookingCheckAvailableService → IRepository                     | Queries the night number data from `IRepository`.                                      |
| 7.1.1 | IRepository → Database                                          | Queries the data from `Database`.                                                      |
| 7.1.2 | Database → IRepository                                          | Returns the data to `IRepository`.                                                     |
| 7.2   | IRepository → IBookingCheckAvailableService                     | Returns the night number data to `IBookingCheckAvailableService`.                      |
| 7.3   | IBookingCheckAvailableService → BookingGmoPaymentCommandHandler | Returns the night number availability to `BookingGmoPaymentCommandHandler`.            |
| 8     | BookingGmoPaymentCommandHandler → IBookingCheckAvailableService | Calls `IsRoomNumber` to check room number availability.                                |
| 8.1   | IBookingCheckAvailableService → IRepository                     | Queries the room number data from `IRepository`.                                       |
| 8.1.1 | IRepository → Database                                          | Queries the data from `Database`.                                                      |
| 8.1.2 | Database → IRepository                                          | Returns the data to `IRepository`.                                                     |
| 8.2   | IRepository → IBookingCheckAvailableService                     | Returns the room number data to `IBookingCheckAvailableService`.                       |
| 8.3   | IBookingCheckAvailableService → BookingGmoPaymentCommandHandler | Returns the room number availability to `BookingGmoPaymentCommandHandler`.             |
| 9     | BookingGmoPaymentCommandHandler                                 | Checks the booking availability.                                                       |
| 9.1   | BookingGmoPaymentCommandHandler → IOrderGmoPaymentService       | Calls `OnlinePaymentRefundAsync` if the booking is not available.                      |
| 9.1.1 | IOrderGmoPaymentService → IBookingReservationService            | Calls `FindOrderIdOfOnlinePaymentAsync` to find the order ID of the online payment.    |
| 9.1.2 | IBookingReservationService → IGmoPaymentGatewayService          | Calls `SearchTradeAsync` to search the trade.                                          |
| 9.1.3 | IGmoPaymentGatewayService → IBookingReservationService          | Returns the trade data to `IBookingReservationService`.                                |
| 9.1.4 | IBookingReservationService → IGmoPaymentGatewayService          | Calls `CancelAsync` to cancel the payment.                                             |
| 9.1.5 | IGmoPaymentGatewayService → IBookingReservationService          | Returns the cancel data to `IBookingReservationService`.                               |
| 9.1.6 | IGmoPaymentGatewayService → BookingGmoPaymentCommandHandler     | Returns a refund successful message if the refund is successful.                       |
| 9.1.7 | IGmoPaymentGatewayService → BookingGmoPaymentCommandHandler     | Returns a refund error message if there is an error in the refund process.             |
| 10    | BookingGmoPaymentCommandHandler → IOrderGmoPaymentService       | Calls `CreateAsync` to create the payment.                                             |
| 10.1  | IOrderGmoPaymentService → IBookingReservationService            | Calls `ConfirmedAsync` to confirm the reservation.                                     |
| 11    | BookingGmoPaymentCommandHandler                                 | Handles the GMO online payment result.                                                 |
| 12    | BookingGmoPaymentCommandHandler                                 | Logs the error if there is a GMO online payment result error.                          |
| 13    | BookingGmoPaymentCommandHandler                                 | Rolls back the transaction if there is a GMO online payment result error.              |
| 14    | BookingGmoPaymentCommandHandler → Client                        | Throws `AppLibertyException` if there is a GMO online payment result error.            |
| 15    | BookingCancellationCommandHandler → MailJobScheduler            | Registers the send mail schedule job.                                                  |
| 15.1  | MailJobScheduler → Reservation.MailService                      | Publishes the mail job to the RabbitMQ queue.                                          |
| 15.2  | MailJobScheduler → BookingCancellationCommandHandler            | Returns the response to `BookingGmoPaymentCommandHandler`.                             |
| 16    | BookingGmoPaymentCommandHandler → ReservationsEndpoint          | Returns a success response to `ReservationsEndpoint`.                                  |
| 17    | ReservationsEndpoint → Client                                   | Returns the final response to the `Client`.                                            |
