...
public abstract class BaseTemplate : IIoTemplate
{
    ...
    protected static Cancellation GetCancellationDetail()
    {
    ...
    protected static string GetOrdinalSuffix(
        int number
    )
    {
    ...
    private string GetCancellationInfoEnDetail()
    {
        ...
        sb.AppendLine($"Cancellation Date: {Reservation?.CancelledDateTime?.AddHours(DefaultValues.TimeZoneOffset):yyyy/MM/dd HH:mm}");
        ...
        sb.AppendLine("Cancellation Fee Collection Method: ");
    ...
    private string GetCancellationInfoJaDetail()
    {
        var sb = new StringBuilder();
        var paymentInfo = Reservation?.PaymentType;
        sb.AppendLine($"キャンセル処理日: {Reservation?.CancelledDateTime?.AddHours(DefaultValues.TimeZoneOffset).ToString("yyyy年MM月dd日 HH時mm分")}");
        sb.AppendLine($"キャンセル料：該当するキャンセル料率: {Reservation?.CancelRateFee}%");
        sb.AppendLine($"キャンセル金額: {Reservation?.CancellationPrice}円");
        sb.AppendLine("キャンセル料徴収方法: ");
