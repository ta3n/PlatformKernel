...
public class BookingReservationPriceData
{
    ...
    public bool? IsPersonsMatch
    {
        get
        {
            if (!MalePersons.HasValue || MalePersons.Value == 0 || !FemalePersons.HasValue || FemalePersons.Value == 0)
            {
                return null;
            }

            var totalPersons = MalePersons.Value + FemalePersons.Value;
            return Persons == totalPersons;
        }
    }
}
