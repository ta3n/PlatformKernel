...
) : IBookingCalendarPriceService
{
    ...
    )
    {
        ...
        foreach (var roomGroupAppDate in inventoryAppDates)
        {
            ...
            var isReceptionAvailable = plan.IsReceptionAvailable(appDateId);
    ...
    )
    {
        ...
        foreach (var roomGroupAppDate in roomGroupAppDates.OrderBy(a => a.AppDateId))
        {
            ...
            var bookingTime = DateTime.UtcNow.AddHours(DefaultValues.TimeZoneOffset).TimeOfDay;
            var checkDay = AppDate.GetId(DateTime.UtcNow.AddHours(DefaultValues.TimeZoneOffset)) < appDatePriceOfPlanResponse.AppDateId;
