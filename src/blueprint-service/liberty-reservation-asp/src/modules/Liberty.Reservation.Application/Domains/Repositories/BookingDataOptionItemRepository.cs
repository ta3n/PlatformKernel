...
) : IBookingDataOptionItemRepository
{
    ...
    )
    {
        ...
        {
            var existingItem = cachedData.Find(
                x => x.PlanId == optionItem.PlanId && x.OptionItemId == optionItem.OptionItemId
            );
            if (existingItem == null)
            {
                cachedData.Add(optionItem);
            }
            else
            {
                // Merge the app dates collections
                var mergedAppDates = existingItem.AppDates
                    .Union(optionItem.AppDates)
                    .ToList();
                var mergedItem = existingItem with { AppDates = mergedAppDates };
                var findIndex = cachedData.FindIndex(
                    x => x.PlanId == optionItem.PlanId && x.OptionItemId == optionItem.OptionItemId
                );
                cachedData[findIndex] = mergedItem;
            }
        }
