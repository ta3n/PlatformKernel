...
public class BookingCreateEndpointRoomGroupNotAlreadyInPlanIntTest : BaseIntegrationTest
{
    private static string BaseUrl => "api/booking";

    private GetBookingData GetBookingData { get; }

    public BookingCreateEndpointRoomGroupNotAlreadyInPlanIntTest()
    {
        GetBookingData = new GetBookingData(Factory, Client);
    }

    [Fact]
    public async Task BookingCreate_RoomGroupNotAlreadyInPlanException()
    {
        var (_, personAgeTypes) = await GetBookingData.GetReservationsAsync();
        var (searchBooking, plan) = await GetBookingData.GetAllBooking();
        _ = await GetBookingData.GetRoomGroup();
        ...
        var url = BaseUrl + $"/plans/{plan.Id}/rooms/10000";
        ...
        try
        {
            var exception = JsonConvert.DeserializeObject<RoomGroupNotAlreadyInPlanException>(json);
