...
public class CheckNightNumberOfRoomEndpointIntTest : BaseIntegrationTest
{
    private GetBookingData GetBookingData { get; }
    private const string BaseUrl = "api/booking";

    public CheckNightNumberOfRoomEndpointIntTest()
    {
        GetBookingData = new GetBookingData(Factory, Client);
    }

    [Fact]
    public async Task CheckNightNumber_ReturnOk_WithResponse()
    {
        ...
        var response = await Client.PostAsync(
            $"{BaseUrl}/rooms/{roomGroup.Id}/check-night-number",
            request
        );
