...
using Liberty.Reservation.Employee.Application.Auth;
...
public static class SecurityStartup
{
    ...
    )
    {
        ...
        app.UseMiddleware<AccessCodeAuthenticationMiddleware>();
...
public class AccessCodeAuthenticationMiddleware(
...
)
{
    ...
    )
    {
        ...
        if (context.Request.Headers.TryGetValue("AccessCode", out var value))
        {
            var accessCode = value.ToString();
            var identitySetting = configuration.GetOptionsExt<IdentitySetting>("Identity");
            var configuredAccessCode = identitySetting.AccessCode?.Code;

            if (accessCode == configuredAccessCode)
            {
                var claims = new[] { new Claim(ClaimTypes.Name, "AccessCodeUser") };
                var identity = new ClaimsIdentity(claims, "AccessCode");
