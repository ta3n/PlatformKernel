...
) : UpdateCommandHandlerBase<BookingChangeExecutionCommand, long>(unitOfWork, null!)
{
    ...
    )
    {
        ...
        EnsureDateInRangeOrThrow(AppDate.GetId(DateTime.UtcNow), payload.CheckInDateId);
    ...
    private static void EnsureDateInRangeOrThrow(
        long bookingDateId,
        long checkInDateId
    )
    {
        if (IsDateOutOfRange(bookingDateId, checkInDateId))
        {
