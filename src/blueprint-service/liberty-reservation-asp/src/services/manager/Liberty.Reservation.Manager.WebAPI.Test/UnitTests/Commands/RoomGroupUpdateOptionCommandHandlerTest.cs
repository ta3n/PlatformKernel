...
public class RoomGroupUpdateOptionCommandHandlerTest
{
    [Fact]
    public async Task HandleAsync_ShouldReturnPlanId_WhenCommandValid()
    {
        ...
        mockMediator.Setup(x => x.Send(It.IsAny<PlanUpdateOptionCommand>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(planId);

        var command = new RoomGroupUpdateOptionCommand(roomGroupId) { Payload = new RoomGroupUpdateOptionRequest(true, []) };
        var handler = new RoomGroupUpdateOptionCommandHandler(
