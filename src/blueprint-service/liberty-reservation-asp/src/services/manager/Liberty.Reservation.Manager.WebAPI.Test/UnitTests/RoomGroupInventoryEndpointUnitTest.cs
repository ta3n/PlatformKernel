...
public class RoomGroupInventoryEndpointUnitTest : BaseUnitTest
{
    protected override void InitData()
    {
        ...
        mockMediator
            ...
            .ReturnsAsync(
                (
                    RoomGroupInventoryGetAllQuery _,
                    CancellationToken _
                ) =>
                {
                    var response = new List<RoomGroupAppDateResponse>
                    {
                        new(20250101, 1, "Room 1", 1, true)
                        {
                            RemainNumber = 1,
                            ReservedNumber = 1
                        },
                        new(20250101, 2, "Room 2", 1, true)
                        {
                            RemainNumber = 1,
                            ReservedNumber = 1
                        }
    ...
    public async Task GetAppDatesOfRoomGroup_ReturnCorrectResult()
    {
        ...
        var response = Assert.IsType<List<RoomGroupAppDateResponse>>(okObjectResult.Value, false);
        Assert.NotNull(headers);
