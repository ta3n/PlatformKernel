...
public class RoomGroupPriceGetStandardPriceQueryHandlerTest
{
    [Fact]
    public async Task HandleAsync_ShouldReturnStandardPrice_WhenQueryValid()
    {
        var facilityId = 1;
        var roomTypeId = 1;
        var siteId = 1;
        var expectedResponse = new PlanRoomDetailStandardPriceResponse(1, 1, 1, 1, 1, []);
        ...
        mockMediator.Setup(m => m.Send(It.IsAny<PlanPriceGetStandardPriceQuery>(), It.IsAny<CancellationToken>()))
        ...
        var query = new RoomGroupPriceGetStandardPriceQuery(roomTypeId, siteId);
        var handler = new RoomGroupPriceGetStandardPriceQueryHandler(
            mockMapper.Object,
            mockMediator.Object,
