        ...
        IPlanRoomGroupSiteAppDateTypePriceService
{
    ...
    )
    {
        ...
        foreach (var removeDataOfSiteInPlanRoom in removeDataOfSiteInPlanRooms)
        {
            var queryable = planRoomGroupSiteAppDatePriceDataRepository
                .GetQueryable()
                .Where(
                    r =>
                        r.RoomGroupId == removeDataOfSiteInPlanRoom.RoomGroupId
                        && r.PlanId == removeDataOfSiteInPlanRoom.PlanId
                        && r.SiteId == removeDataOfSiteInPlanRoom.SiteId
                        && r.PriceData!.PersonMin == removeDataOfSiteInPlanRoom.PriceData!.PersonMin
                        && r.PriceData!.PersonMax == removeDataOfSiteInPlanRoom.PriceData!.PersonMax
                );

            await queryable.ExecuteDeleteAsync(cancellationToken);
        }
