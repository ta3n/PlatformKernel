...
public class KakusanGetAllRoomTypeQueryHandlerTest
{
    private readonly KakusanGetAllRoomTypeQueryHandler _handler;

    public KakusanGetAllRoomTypeQueryHandlerTest()
    {
        ...
        var configSettingMock = new Mock<IOptions<C001Setting>>();
        ...
        configSettingMock
            .Setup(x => x.Value)
            .Returns(new C001Setting());

        _handler = new KakusanGetAllRoomTypeQueryHandler(
            mapperMock.Object,
    ...
    {
        // Arrange
        var request = new RoomTypeRequest { HotelIds = [] };

        var query = new KakusanGetAllRoomTypeQuery(request);
