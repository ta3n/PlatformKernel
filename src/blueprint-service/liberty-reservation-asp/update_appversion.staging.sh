...
if [ -z "$1" ]; then
  echo "❌ Please provide a new version, e.g.: ./update_appversion.staging.sh 1.2.3 [-y]"
  exit 1
fi
...
IGNORED_PATHS=(
  "./src/modules/Liberty.Reservation.AppHost/appsettings.Staging.json"
)

echo "🔍 Searching for appsettings.Staging.json files (excluding bin/, obj/, and ignored files)..."

# Find all appsettings.Staging.json files excluding bin/ and obj/
ALL_FILES=$(find ./src -type f -name "appsettings.Staging.json" | grep -v "/bin/" | grep -v "/obj/")
...
if [ ${#FILTERED_FILES[@]} -eq 0 ]; then
  echo "❌ No valid appsettings.Staging.json files found after filtering."
  exit 1
fi
