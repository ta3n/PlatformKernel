...
if [ -z "$1" ]; then
  echo "❌ Please provide a new version, e.g.: ./update_appversion.sh 1.2.3 [-y]"
  exit 1
fi
...
IGNORED_PATHS=(
  "./src/modules/Liberty.Reservation.AppHost/appsettings.json"
)

echo "🔍 Searching for appsettings.json files (excluding bin/, obj/, and ignored files)..."

# Find all appsettings.json files excluding bin/ and obj/
ALL_FILES=$(find ./src -type f -name "appsettings.json" | grep -v "/bin/" | grep -v "/obj/")
...
if [ ${#FILTERED_FILES[@]} -eq 0 ]; then
  echo "❌ No valid appsettings.json files found after filtering."
  exit 1
fi
