...
- `src/BlueprintNormal/ClientApp/` - Web Application folder
...
In ./src/BlueprintNormal/ClientApp run
...
auto-refreshes when files change on your hard drive.
    ...
    npm --prefix ./src/BlueprintNormal/ClientApp start

npm is also used to manage CSS and JavaScript dependencies used in this application. You can upgrade dependencies by
specifying a newer version in [package.json](package.json). You can also run `npm update` and `npm install` to manage dependencies.
Add the `help` flag on any command to see how you can use it. For example, `npm help update`.

The `npm --prefix ./src/BlueprintNormal/ClientApp run` command will list all of the scripts available to run for this project.
Before you can build this project, you must install and configure the following dependencies on your machine:
...
The service worker initialization code is commented out by default. To enable it, uncomment the following code in `src/BlueprintNormal/ClientApp/src/index.html`:
...
To build the artifacts and optimize the BlueprintNormal application for production, run:

```
cd ./src/BlueprintNormal
rm -rf ./src/BlueprintNormal/wwwroot
dotnet publish --verbosity normal -c Release -o ./app/out ./BlueprintNormal.csproj
```

The `./src/BlueprintNormal/app/out` directory will contain your application dll and its depedencies.
...
3. Run ``dotnet sonarscanner begin /d:sonar.login=admin /d:sonar.password=admin /k:"BlueprintNormal" /d:sonar.host.url="http://localhost:9001" /s:"`pwd`/SonarQube.Analysis.xml"``
...
```bash
docker build -f ./Dockerfile-Back -t blueprintnormal .
```

Then run:

```bash
docker run -p 8080:8080 blueprintnormal
```
